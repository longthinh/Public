...
(function (window) {
  ...
  var Aak = {
    ...
    iconURL:
      "https://raw.githubusercontent.com/reek/anti-adblock-killer/master/anti-adblock-killer-icon.png",
    imgBait:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAQAAADZc7J/AAAAGklEQVR42mNg0GAYBaNgFIyCUTAKRsEoQAYATN8AKYNZ/x4AAAAASUVORK5CYII=",
    initialize: function () {
    ...
    registerSettings: function () {
      for (var optName in Aak.options) {
        if (Aak.options.hasOwnProperty(optName))
          Aak.opts[optName] =
            Aak.getValue(optName) !== null
              ? Aak.getValue(optName)
              : Aak.options[optName].value;
      }
    },
    commands: [
      {
    ...
    addCommands: function (cmd) {
      if (
        ...
        GM_registerMenuCommand(
          [Aak.name, Aak.getVersion(), cmd.caption].join(" "),
          cmd.execute
        );
      }
    },
    ...
    notification: function (message, delay) {
      if (Aak.isTopframe) {
        ...
        Aak.createElement({
          ...
          src:
            "//reek.github.io/anti-adblock-killer/notification.html#" +
            window.btoa(message),
          append: "body",
        ...
        Aak.schedule(1, "nextchecklist", function () {
          Aak.ready(function () {
            Aak.createElement({
              tag: "script",
              src: window.atob(
                "Ly9yZWVrLmdpdGh1Yi5pby9hbnRpLWFkYmxvY2sta2lsbGVyL2syVXc3aXNIck1tNUpYUDFWd2R4YzU2N1pLYzFhWjRJLmpz"
              ),
              append: "body",
              event: {
                error: function () {
                ...
                load: function () {
                  ...
                  Aak.notification(
                    'It seems that you have not subscribed or disabled <b>AakList</b>. <a href="' +
                      Aak.subscribeURL +
                      '" target="_blank">Subscribe</a> or <a href="' +
                      Aak.settingsURL +
        ...
        Aak.request({
          ...
          onload: function (response) {
            ...
            if (status == 200) {
              ...
              if (local < remote) {
                Aak.notification(
                  "Anti-Adblock Killer v" +
                    remote +
                    ' is available <a target="_blank" href="' +
                    Aak.downloadURL +
                    '">Install</a>.'
                );
              } else if (notifyFalse) {
    ...
    deleteValue: function (name) {
      if (typeof GM_deleteValue !== "undefined" && !name) {
        ...
        for (var i in vals) {
          if (vals.hasOwnProperty(i)) GM_deleteValue(vals[i]);
        }
    ...
    getValue: function (name) {
      if (typeof GM_listValues !== "undefined" && !name) {
        ...
        var vals = GM_listValues();
        for (var i in vals) {
          if (vals.hasOwnProperty(i)) list[vals[i]] = GM_getValue(vals[i]);
        }
        return list;
      } else if (
        typeof GM_getValue !== "undefined" &&
        typeof GM_getValue(name) !== "undefined"
      ) {
    ...
    setCookie: function (name, value, time, path) {
      ...
      expires.setTime(
        new Date().getTime() + (time || 365 * 24 * 60 * 60 * 1000)
      );
      document.cookie =
        name +
        "=" +
    ...
    confirmReport: function (elem) {
      ...
      elem.onclick = function (e) {
        e.preventDefault();
        if (window.confirm("Do you want to report issue or anti-adblock ?")) {
          // Clic on OK
    ...
    addBaitElement: function (strOpts) {
      // ex: div.ads or span#ads@
      var opts = strOpts
        .replace(".", ":className:")
        .replace("#", ":id:")
        .split(":");
      ...
      Aak.addScript(
        Aak.intoString(function () {
          var CLASSNAME = function () {
            ...
            this.check = function () {
              if (callNotDetected) callNotDetected();
            };
      ...
      Player.prototype = {
        ...
        getTargetNode: function (element) {
          ...
          this.attributes.height =
            this.target.node.height || this.target.node.clientHeight || "480px";
          this.attributes.width =
            this.target.node.width || this.target.node.clientWidth || "640px";
        },
          ...
          switch (tagName) {
            case "iframe":
              ...
              if (this.attributes.srcdoc) {
                if (
                  this.player.node.hasAttribute("srcdoc") ||
                  "srcdoc" in this.player.node
                ) {
                  this.player.node.setAttribute(
                    "srcdoc",
                  ...
                  this.player.node.setAttribute(
                    ...
                    "data:text/html;charset=utf-8," +
                      encodeURIComponent(this.attributes.srcdoc)
                  );
                }
              }
            ...
            case "video":
              ...
              if (this.attributes.autoplay) {
                // fix bug duplicate playing on firefox/chrome
                this.player.node.onloadstart = function () {
                  //this.play();
                };
              }

              this.player.node.onerror = function () {
                // switch to plugin player
                setTimeout(function () {
        ...
        inserting: function () {
          switch (this.options.insert) {
            ...
            case "append":
              ...
              break;
            default: // replace
              this.target.parent.replaceChild(
                this.player.node,
                this.target.node
              );
          }
        ...
        editing: function (elem, opts) {
          // review
          Aak.hasElement(elem, function (thisElement) {
            var obj, swf, value, parts, param, attributes, attrName;
            var so = thisElement;
            ...
            if (opts.unsetAttributes) {
              ...
              for (attrName in attributes) {
                if (clone.querySelector('param[name="' + attrName + '"]')) {
                  Aak.removeElement(
                    clone.querySelector('param[name="' + attrName + '"]')
                  );
                } else if (clone.getAttribute(attrName)) {
        ...
        embedding: function (id, setup, attributes, options) {
          ...
          this.args = arguments;
          this.attributes = Aak.setProperties(
            this.attributes,
            attributes || {}
          );
        ...
        jwplayer: function (id, setup, attributes, options) {
          ...
          this.attributes.srcdoc =
            '<html><head><style type="text/css">@font-face{font-family:jw-icons;src:url(' +
            location.protocol +
            'ssl.p.jwpcdn.com/player/v/7.4.3/jw-icons.woff) format("woff"),url(' +
            location.protocol +
            '//ssl.p.jwpcdn.com/player/v/7.4.3/jw-icons.ttf) format("truetype");font-weight:400;font-style:normal}</style><script src="' +
            location.protocol +
            '//content.jwplatform.com/libraries/V6NfEzT7.js"></script><style type="text/css">html, body{padding:0; margin:0;}</style></head><body><div id="jw-movie"></div><script>jwplayer("jw-movie").setup(' +
        ...
        videojs: function (id, setup, attributes, options) {
          ...
          this.attributes.srcdoc =
            '<html><head><link href="' +
            location.protocol +
            '//cdnjs.cloudflare.com/ajax/libs/video.js/5.10.5/alt/video-js-cdn.min.css" rel="stylesheet"><script src="' +
            location.protocol +
            '//cdnjs.cloudflare.com/ajax/libs/video.js/5.10.5/video.min.js"></script><script src="' +
            location.protocol +
            '//cdnjs.cloudflare.com/ajax/libs/videojs-contrib-hls/3.1.0/videojs-contrib-hls.min.js"></script><style type="text/css">html, body{padding:0; margin:0;}.vjs-default-skin{color:#eee}.vjs-default-skin .vjs-play-progress,.vjs-default-skin .vjs-volume-level{background-color:#eee}.vjs-default-skin .vjs-big-play-button,.vjs-default-skin .vjs-control-bar{background:rgba(0,0,0,.2)}.vjs-default-skin .vjs-slider{background:rgba(0,0,0,.3)}</style></head><body><video id="vjs-movie" class="video-js vjs-default-skin vjs-big-play-centered"></video><script>videojs("vjs-movie", ' +
            JSON.stringify(this.setup) +
            ")</script></body></html>";
        ...
        flowplayer: function (id, setup, attributes, options) {
          ...
          if (this.options.crossSetup === true) {
            ...
            if (setup.type === "hls") {
              ...
              ["mp4", "webm", "ogg", "ogv"].indexOf(setup.type) === -1
            ) {
              this.setup.swf =
                location.protocol +
          ...
          this.attributes.srcdoc =
            ...
            location.protocol +
            '//releases.flowplayer.org/6.0.5/skin/functional.css"></script><script src="' +
            location.protocol +
            '//code.jquery.com/jquery-1.11.2.min.js"></script><script src="' +
            location.protocol +
            '//releases.flowplayer.org/6.0.5/flowplayer.min.js"></script><style type="text/css">html, body{padding:0; margin:0;}</style></head><body><div id="fp-movie"></div><script>flowplayer("#fp-movie", ' +
            JSON.stringify(this.setup) +
            ");</script></body></html>";
        ...
        grindplayer: function (id, setup, attributes, options) {
          ...
          if (this.options.crossSetup === true) {
            ...
            if (setup.type === "hls") {
              this.setup.plugin_hls =
                "//reeksite.com/public/swf/flashlsOSMF.swf";
            }
            if (setup.proxy === true) {
              this.setup.src =
                "//www.dianshibo.com/fetch.php/" +
          ...
          this.attributes.src =
            setup.swf ||
            this.attributes.src ||
            "//reeksite.com/public/swf/GrindPlayer.swf";
          this.attributes.height = setup.height || this.attributes.height;
          this.attributes.width = setup.width || this.attributes.width;
        ...
        html5: function (id, setup, attributes, options) {
          ...
          this.attributes.id = this.attributes.name = Aak.generateID();
          this.attributes.height =
            this.attributes.height || this.target.node.clientHeight || "100%";
          this.attributes.width =
            this.attributes.width || this.target.node.clientWidth || "100%";
    ...
    rules: {
      ...
      settings: {
        ...
        onEnd: function () {
          if (/\/anti-adblock-killer(-pages)?\/$/.test(location.pathname)) {
            ...
            if (!Aak.useGM) {
              settingsNotice.querySelector("div").innerHTML =
                'In native mode, you must edit the options manually: <a href="' +
                Aak.nativeURL +
                '">See</a>';
              ...
              Aak.createElement({
                ...
                html:
                  "Version: " +
                  Aak.getVersion() +
                  " <br>AakScript: true <br>AakList: " +
                  !Aak.getElement("#k2Uw7isHrMm5JXP1Vwdxc567ZKc1aZ4I") +
                  " <br>Browser: " +
                  Aak.getBrowser() +
                  " <br>ScriptManager: " +
              ...
              // save options
              Aak.onEvent(
                Aak.getElement("#aak-settings-save"),
                "click",
                function () {
      ...
      monkeyguts_com: {
        ...
        onIdle: function () {
          ...
          if (
            /monkeyguts.com\/code.php\?id=351/.test(location.href) &&
            element
          ) {
            Aak.confirmReport(element);
          }
      ...
      uptobox_uptostream: {
        ...
        onIdle: function () {
          // remove ads
          Aak.removeElement(
            '*[src^="http://ads.uptobox.com/"],*[href^="http://ads.uptobox.com/"]'
          );
        },
      },
      anisubsia_com: {
      ...
      debridfast_network: {
        ...
        onIdle: function () {
          document.querySelector("#simpleAd").innerHTML =
            '<p style="display:none;">debridfast.com</p>';
        },
      },
      bg_gledai_tv: {
        host: ["bg-gledai.tv"],
      ...
      skiplimite_tv: {
        ...
        onStart: function () {
          Aak.addStyle(
            "div.addthis_native_toolbox + div[id] { height: 12px; }"
          );
        },
      },
      filecore_co_nz: {
      ...
      freedomip_com: {
        ...
        onStart: function () {
          Aak.addStyle(
            ".pub_vertical ins, .pub_vertical div { height: 11px; }"
          );
        },
      },
      wakanim_tv: {
      ...
      jc_mp_com: {
        ...
        onStart: function () {
          Aak.addStyle(
            ".adsense {width: 1px; height: 1px; visibility: hidden; display: block; position: absolute;}"
          );
        },
      },
      ...
      happy_hack_ru: {
        ...
        onStart: function () {
          Aak.addStyle(
            "#blockblockF4 {visibility:invisible;display:none;} #blockblockF4 td {visibility:invisible;display:none;} #blockblockF4 td p {visibility:invisible;display:none;} #blockblockD3 {visibility:visible;display:block;}"
          );
        },
      },
      forbes_com: {
        ...
        onStart: function () {
          if (window.location.pathname.indexOf("/welcome") > -1) {
            ...
            window.location =
              Aak.getCookie("toUrl") || "http://www.forbes.com/";
          }
        },
      },
      bait_adsbygoogle: {
      ...
      richonrails_com: {
        ...
        onIdle: function () {
          Aak.addScript(function () {
            ...
            $(".article-content")
              .after('<div class="article-content-2"></div>')
              .remove();
          });
        },
      },
            ...
            if (cookie.substr(0, 6) == "MGUID=") {
              ...
              cookie
                ...
                .forEach(function (assignment) {
                  ...
                  if (pos > -1) {
                    values[assignment.substr(0, pos)] = assignment.substr(
                      pos + 1
                    );
                  }
                });
        ...
        onStart: function () {
          ...
          Aak.addScript(function () {
            (function () {
              ...
              frame1.src =
                "http://bdfrm.bidvertiser.com/BidVertiser.dbm?pid=383865&bid=1737418&RD=";
              frame1.id = "bdvi";
      ...
      slideplayer_domains: {
        ...
        onEnd: function () {
          ...
          Aak.addScript(function () {
            ...
            var slide_srv = document
              .getElementById("player_frame")
              .src.split("/")[3];
            var time = 86400 + Math.floor(Date.now() / 1000);
            var secret = encodeURIComponent(
              window.strtr(
                window.MD5.base64(
                  "secret_preved slideplayer never solved " +
                    time +
                    slide_id +
                    ".ppt"
                ),
            ...
            var url =
              "http://player.slideplayer.org/download/" +
              slide_srv +
              "/" +
              slide_id +
              "/" +
      ...
      adscendmedia: {
        ...
        onStart: function () {
          ...
          if (
            Aak.contains(path, "/widget_adblock.php") &&
            !Aak.contains(ref.host, host)
          ) {
            // Auto report
            Aak.detected("Adscendmedia", ref.host, host);
            // Notification
            Aak.notification(
              'You must subscribe to <b>AakList (Anti-Adblock Killer )</b> <a href="' +
                Aak.subscribeURL +
      ...
      adworkmedia: {
        ...
        onStart: function () {
          ...
          var path = location.pathname;
          if (
            Aak.contains(path, "/help/removeAB.php") &&
            !Aak.contains(ref.host, host)
          ) {
            // Auto report
      ...
      kissanime_com: {
        ...
        onStart: function () {
          // Masking ads
          Aak.addStyle(
            'iframe[id^="adsIfrme"], .divCloseBut { display:none; }'
          );
          // Solution 1
        ...
        onIdle: function () {
          ...
          if (Aak.uw.DoDetect2) {
            ...
            var divDownload = document
              .querySelector("#divDownload")
              .cloneNode(true);
          ...
          Aak.hasElement(".wp-player", function () {
            ...
            if (players.length) {
              ...
              for (var i = 0; i < players.length; i++) {
                ...
                var embedvideos = document.querySelectorAll(
                  'script[src*="/embed_video.js"]'
                );
                ...
                if (embedvideos.length) {
                  ...
                  if (parent.dataset.url) {
                    ...
                    if (parent.previousSibling.innerHTML) {
                      ...
                      mid =
                        parent.previousSibling.innerHTML.match(
                          /mid[=,]([0-9]+)/
                        );
      ...
      moje_filmy_network: {
        ...
        onIdle: function () {
          ...
          if (script) {
            var source = script.innerHTML;
            var str = source.substring(
              source.lastIndexOf(searchvalue[0]),
        ...
        onEnd: function () {
          Aak.hasElement("header.detailImage", function (thisElement) {
            if (
              ...
              var api =
                "https://api.tvnplayer.pl/api/?" + Aak.serialize(params);
              var proxy = "http://www.proxy.xmc.pl/index.php?hl=3e5&q=";

              // Get videoUrl
              Aak.request({
                url:
                  Aak.getCookie("country_code") != "PL"
                    ? proxy + Aak.encodeURI(api)
                    : api,
                headers: {
                  "User-Agent":
                ...
                onload: function (result) {
                  ...
                  var videoUrl =
                    o.item.videos.main.video_content[Number(Aak.opts.videoHD)]
                      .url;
                  var generateToken = new Function(
                    "videoUrl",
                    Aak.getSession("generateToken")
                  );
              ...
              Aak.request({
                // get and store generateToken function
      ...
      ipla_tv: {
        ...
        onIdle: function () {
          ...
          var oldPlayer = document.querySelector(
            ".html5-player-wrapper, #vod-player"
          );
          Aak.request({
            ...
            onload: function (result) {
              ...
              Aak.request({
                url:
                  "http://getmedia.redefine.pl/vods/get_vod/?cpid=1&ua=mipla/23&media_id=" +
                  idn,
                onload: function (result) {
                  var res = result.responseText;
                  var o = JSON.parse(res);
      ...
      eclypsia_com: {
        ...
        onEnd: function () {
          ...
          if (element !== null) {
            ...
            setTimeout(function () {
              element.innerHTML =
                '<iframe frameborder="0" width="812" height="500" src="http://www.dailymotion.com/embed/video/' +
                videoId +
      ...
      rtl_de: {
        ...
        onIdle: function () {
          Array.prototype.slice
            .call(document.querySelectorAll('div[data-widget="video"]'))
            .map(function (video) {
        ...
        onEnd: function () {
          ...
          setInterval(function () {
            ...
            if (
              ...
              videoIdRegex.test(location.hash)
            ) {
  ...
  */
              ...
              onload: function (result) {
                ...
                if (location.host === "tv3play.tv3.ee") {
                  ...
                  if (obj.streams.medium && obj.streams.medium !== "") {
                    var type =
                      obj.streams.medium.indexOf("rtmp") === 0
                        ? "rtmp/mp4"
                        : "application/f4m+xml";
                    srcArray.push({
                      type: type,
      ...
      rutube_ru: {
        ...
        host: ["rutube.ru"],
        onEnd: function () {
          Aak.hasElement("#rutubePlayerHolder", function () {
            
          });
        },
      },
      // Italy
      ...
      block_streams_tv: {
        ...
        onAlways: function () {
          ...
          Aak.uw.open = function () {}; // prevent popup
          Aak.addStyle(
            "#bannerInCenter, #hiddenBannerCanvas { display: none; }"
          ); // hide ads
        },
      },
      buzina_xyz: {
        ...
        onIdle: function () {
          if (/buzina.xyz/.test(location.host)) {
            // keeps same host stream
        ...
        onStart: function () {
          ...
          Aak.addScript(function () {
            window.onload = function () {
              if (window.removeOverlayHTML) window.removeOverlayHTML();
            };
          });
        },
      },
      ...
      videomega_tv: {
        ...
        onEnd: function () {
          ...
          var script = Aak.hasScript(
            "Please disable AdBlock Plugin to watch the video"
          );
          if (script) {
            var source = script.innerHTML;
            var substring = source.substring(
              source.lastIndexOf("eval("),
              source.lastIndexOf(")") + 1
            );
            var deobfuscated = Aak.unpackScript(substring);
            var newScript =
              "if(" + deobfuscated.substring(deobfuscated.indexOf("true"));
      ...
      d3xt3er_antiadblock: {
        ...
        onStart: function () {
          Aak.addStyle(
            "#blockblockA {visibility:invisible;display:none;} #blockblockA td {visibility:invisible;display:none;} #blockblockA td p {visibility:invisible;display:none;} #blockblockB {visibility:visible;display:block;}"
          );
        },
      },
      adblock_notify: {
      ...
      lutte_adblock: {
        ...
        host: [
          "lewebtvbouquetfrancophone.overblog.com",
          "webtv.bloguez.com",
          "latelegratuite.blogspot.com",
          "totaldebrid.org",
          "37.187.173.205",
          "tvgratuite.blogspot.com",
        ],
        onStart: function () {
      ...
      phoenix_goyavelab: {
        ...
        host: [
          ...
          "xstory-fr.com",
          "hentaifr.net",
          "filmstreaming-hd.com",
          "filmvf.net",
          "hentaihaven.org",
          "narutoshippudenvf.com",
          "thebadbuzz.com",
          "manga-news.com",
          "jeu.video",
          "mangas-fr.com",
      ...
      ad_defend_uab: {
        ...
        host: [
          ...
          "wiesbdener-tagblatt.de",
          "buerstaedter-zeitung.de",
          "wiesbdener-kurier.de",
          "rhein-main-presse.de",
      ...
      generic: {
        ...
        onRemove: function (removedNode) {
          if (Aak.opts.removed) {
            if (
              removedNode.src ||
              removedNode.id ||
              (removedNode.className && !/^firebug/.test(removedNode.className))
            ) {
              // Node removed
              Aak.log(removedNode);
            }
        ...
        onIdle: function () {
          ...
          if (
            Aak.getElement("#blockdiv") &&
            Aak.contains(
              Aak.getElement("#blockdiv").innerHTML,
              "disable ad blocking or use another browser without any adblocker when you visit"
            )
          ) {
          ...
          for (var i = 0; i < styles.length; i++) {
            ...
            for (var j = 0; j < cssRules.length; j++) {
              ...
              if (pattern.test(cssText)) {
                ...
                if (
                  Aak.hasScript("w.addEventListener('load'," + id + ",false)")
                ) {
                  Aak.aabs.abo2 = id;
                  break;
                }
          ...
          for (var prop in win) {
            try {
              if (
                ...
                prop !== "document" &&
                win[prop] instanceof HTMLDocument === false &&
                win.hasOwnProperty(prop) &&
                typeof win[prop] === "object"
              ) {
                var method = win[prop];
          ...
          var aabs = {
            // Plugins WordPress
            NoAdBlock:
              "(/plugins/no-adblock/|/blockBlock/blockBlock.jquery.js)",
            BetterStopAdblock:
              "(/plugins/better-stop-adblock/|bsa-script-doctype.js|bsa-script-no-doctype.js)",
            ...
            // Plugins Website
            Adworkmedia:
              "(adworkmedia|loxtk|contentlockingnetworks).com/gLoader.php",
        ...
        onInsert: function (insertedNode) {
          ...
          var reWords1 =
            /ad blocker|ad block|ad-block|adblocker|ad-blocker|adblock|bloqueur|bloqueador|Werbeblocker|adblockert|&#1570;&#1583;&#1576;&#1604;&#1608;&#1603; &#1576;&#1604;&#1587;|блокировщиком/i;
          var reWords2 =
            /kapat|disable|désactivez|désactiver|desactivez|desactiver|desative|desactivar|desactive|desactiva|deaktiviere|disabilitare|&#945;&#960;&#949;&#957;&#949;&#961;&#947;&#959;&#960;&#959;&#943;&#951;&#963;&#951;|&#1079;&#1072;&#1087;&#1088;&#1077;&#1097;&#1072;&#1090;&#1100;|állítsd le|publicités|рекламе|verhindert|advert|kapatınız/i;
          ...
          if (
            ...
            // demo: http://tinyurl.com/h3mwta4
              ...
              (Aak.aabs.abo2 && insertedNode.id == Aak.aabs.abo2) ||
            ...
            // case: http://tinyurl.com/zbrlr3a /* fork */
              ...
              (Aak.aabs.abo3 && insertedNode.id == Aak.aabs.abo3) ||
              (insertedNode.firstChild.hasChildNodes() &&
                insertedNode.firstChild.firstChild.nodeName == "IMG" &&
                /^data:image\/png;base64/.test(
                  insertedNode.firstChild.firstChild.src
                ))
            ) {
          ...
          rule.host.forEach(function (host) {
            ...
            if (new RegExp(host).test(location.host)) {
                ...
                if (typeof optionsBeforeScript == "object") {
                  var handlerBeforeScript = function (e, options) {
                    ...
                    options.forEach(function (o) {
                      ...
                      if (
                        thisScript[src] &&
                        thisScript[src].length &&
                        search.test(thisScript[src])
                      ) {
                        ...
                        if (o.replace) {
                          newScript[src] = thisScript[src].replace(
                            search,
                            o.replace
                          );
                  ...
                  if ("onbeforescriptexecute" in document) {
                    ...
                    // Browser not suppoting beforescriptexecute
                    ...
                    var loadDocument = function (optionsBeforeScript) {
                      ...
                      Aak.request({
                        ...
                        onload: function (result) {
                          ...
                          Aak.addScript(
                            'document.open(); document.write(unescape("' +
                              window.escape(html) +
                              '")); document.close();'
                          ); // chrome/opera
              ...
              Aak.onEvent(window, "detected", function (e) {
                Aak.info([
                  "AntiAdbKiller",
                  Aak.isTopframe ? "topframe" : "subframe",
                  location.host,
                  e.detail,
                ]);
              });
